...
test('it shows the title in the banner', () => {
	setup();
	// We expect that the title 'Hi, I am Rimsha' is in the banner component
	expect(screen.getByText(/Hi, I am Rimsha/i)).toBeInTheDocument();
});
...
test('can download cv when clicked on download cv button', async () => {
	...
	// expect(downloadLink.download).toEqual('RIMSHA-Resume.pdf');
	// expect(downloadLink.href).toEqual('/files/RIMSHA-Resume.pdf');
